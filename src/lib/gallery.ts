...
/** Public build gallery backed by GitHub Issues — no server needed. Any open issue whose body contains a ```build code block
 *  counts (the label is only cosmetic: GitHub ignores `labels=` for users without triage rights).
...
export async function fetchGallery(): Promise<GalleryEntry[]> {
  const res = await fetch(`https://api.github.com/repos/${REPO}/issues?state=open&per_page=100&sort=created`, { headers: { Accept: 'application/vnd.github+json' } });
  if (!res.ok) throw new Error(`GitHub ${res.status}`);
  const issues: any[] = await res.json();
  const out: GalleryEntry[] = [];
  for (const i of issues) {
    if (i.pull_request) continue;
    const m = (i.body || '').match(CODE_RE);
