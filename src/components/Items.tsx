...
const SECTIONS: [string, string][] = [['essences', 'Essências'], ['potions', 'Poções'], ['coins', 'Moedas'], ['materials', 'Materiais']];
...
function ItemTip({ it }: { it: Item }) {
  ...
  return <div className="ud-card"><div className="ud-head" style={{ borderColor: c }}>{it.icon && <img src={iconUrl(it.icon)} alt="" />}<div><div className="ud-name" style={{ color: c }}>{it.name}</div><div className="ud-sub">{[it.rarity, it.useOn.join(', ')].filter(Boolean).join(' · ')}</div></div></div>
    ...
    {it.howToGet.length > 0 && <div className="ud-foot">Source: <span className="ud-src">{it.howToGet.join(' ')}</span></div>}</div>;
}
...
export function Items() {
  ...
  return <section className="panel">
    <div className="row between"><h2>Itens <small className="muted">{SECTIONS.map(([k, l]) => `${l} ${misc[k]?.length ?? 0}`).join(' · ')}</small></h2><input placeholder="Buscar…" value={q} onChange={e => setQ(e.target.value)} /></div>
    <nav className="subnav">{SECTIONS.map(([k, l]) => <button key={k} className={sec === k ? 'on' : ''} onClick={() => setSec(k)}>{l}</button>)}</nav>
    {list.length === 0 && <p className="muted">Sem dados ainda — a carga do banco (workflow "Update database") preenche esta seção.</p>}
