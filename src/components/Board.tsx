...
import { analyzeBoard, cells, CENTER, dirAngle, hexPos, runeColor, triggerSpec } from '../lib/rules';
...
export function Board({ build, set }: { build: Build; set: (b: Build) => void }) {
  ...
  const update = (cell: string, patch: Partial<{ rune?: string; runestone?: string; dir?: number }>) => {
    const board = { ...build.board, [cell]: { ...build.board[cell], ...patch } };
    if (!board[cell].rune && !board[cell].runestone) delete board[cell];
    else if (board[cell].dir === undefined || board[cell].dir === 0) delete board[cell].dir;
    set({ ...build, board });
  };
  ...
  return (
    <section className="panel">
      ...
      {pick && pick.kind === 'rune' && <Picker title={t('pickRune')} allowClear onClose={() => setPick(null)}
        ...
        filters={[{ label: t('type'), options: ['Skill Rune', 'Link Rune', 'Trigger Rune'], match: (i, v) => i.sub === v }, { label: t('tag'), options: tags, match: (i, v) => !!i.tags?.includes(v) }]}
