    ...
    <section className="panel">
      ...
      <div className="groups">
        {groups.map(g => <div key={g.cell} className="group">
          ...
          {g.links.map(l => <div key={l.cell} className={l.check.ok ? 'ok' : 'bad'}><Tip content={<RuneTip r={l.rune} level={level} bonus={bonus} />}><img src={l.rune.icons[0]} alt="" /></Tip> {l.rune.name} — <small>{l.check.key ? t(l.check.key, { tags: l.check.tags ?? '' }) : l.check.reason}</small></div>)}
        </div>)}
