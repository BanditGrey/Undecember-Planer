...
- **Gear rara, Charms, Relíquias, Joias & Lacrima** – além dos únicos/autoridades, cada slot aceita uma peça rara/craftada com afixos em texto livre (um por linha); a aba Equipamento ganhou um painel para até 12 charms, 6 relíquias, 8 joias e 3 Lacrimas (tipo de slot, grau, taxa de absorção). Não existe base pública estruturada para esses itens, então são texto — mas linhas de dano (`+X% Fire DMG`, `X% DMG Amplification`…) entram na estimativa de DPS (Lacrima multiplicada pela absorção) e tudo viaja no link da build.
